...
| RS1（RS 纯函数层①——lib/util/origin/matcher/adblock） | d4ef999 | RS-051..069 19 项：pipeline 全脚本确定性+九阶段声明顺序（051）/ABI 探测幂等+fail-closed 门禁契约（052）/js_inject build Vec join 注释锁定+空管线测（053，format! 链已被 RS-040 消化）/util 协议相对 //host 修复（此前空串）+userinfo 变体+大写 scheme+未闭合方括号 fail-closed（此前带 [ 伪主机名）+IPv6 形态+纯端口 8 用例（054/055）/extract_host Cow 评估——uniffi checksum 5061 锁签名，分配消除走调用侧（056）/origin 非点分 IPv4 三拒+4 段保留+8192 恰边界+端口 1/65535/65536 7 用例（057/058）/unwrap_or_default 消除（059）/双 clone 消除 origin 先构造后 move（060）/matcher Any1 flat 门控+unicode 5 用例（061）/恰 16384/16385 双入口边界（062）/covers 乘积上限 4M 格补齐——RS-016 只堵输入上限，两侧近 16K 时 268MiB 分配洞/ASCII 字节快路径 DP 泛型化 run_match\<T\>（063）/subsumes 缓存评估——违背无状态约束维持现状（064）/adblock 端口/大写/裸 host+畸形 URL fail-closed 6 用例（065）/TLD 全网拦截语义文档锁定（066）/new 补 ///（067）/derive(Debug)（068）/should_block_host 预归一 API+should_block 委托（069） | 19 | cargo 273+5+4（+15）；clippy 0 警告；五门禁全绿 |
| RS2（RS 指纹防护模块②——八防护模块） | 211c745 | RS-070..084 15 项：query_strip strip 重写先分离 fragment 再定位 query——修复 fragment 内 '?' 被误当 query 分隔（path#a?fbclid=x 遭错误剥离）+空段过滤+fragment '?'/空 query/flag 形态 5 用例（070）/TRACKING_PARAMS Cow 静态切片零分配（071）/自定义字体转义+通用族 2 用例（072）/microseconds=0 饱和+jitter 透传 2 用例（073）/补 mark/measure/rAF 覆盖——startTime/duration 直取宿主内部时钟不经 JS 可见 performance.now，独立圆整（074）/自定义视口透传+Int32Array 契约 2 用例（075）/viewport 数组缓存单实例——对象身份漂移可被 Object.is 检测（076）/扩展枚举面摘除——getSupportedExtensions 过滤 debug_renderer_info+getExtension null（077）/step=0 钳制+roundTo minV 2 用例（078）/colorDepth/pixelDepth 固定 24+DPR 0.25 圆整（079）/toLocaleString 独立欺骗通道+注册接口防篡改 2 用例（080）/Debug 不泄种子+脚本确定性+seed_bytes 同源 3 用例（081）/canvas 读取三通道全覆盖——toBlob+OffscreenCanvas.convertToBlob 此前漏防=无噪声原图；Audio 归属 PerSiteSeed 文档化（082）/空域名/Unicode total+大小写按原文派生 2 用例（083）/derive_hex write! 单缓冲（084） | 15 | cargo 294+5+4（+21）；clippy 0 警告；五门禁全绿 |
| RS3（RS 策略层③——ext_proxy/decision/policy/action_policy/executor） | f90f7b8 | RS-085..099 15 项：endpoint 转义回归（085）/with_endpoint 校验 http(s) scheme——javascript:/相对路径 fail-closed 退化禁用（086）/拦截面扩充——补 googleusercontent crx 媒体通道+chromewebstore 站内请求（087）/js_inject 多阶段顺序+禁用剔除 2 用例（088）/decision 全模块 3 用例（089）/is_allow/deny_code/requires_confirmation helper 单源（090）/PolicyEngine+DefaultLocalPolicy new 补 ///（091）/trait 方法语义文档 None=无匹配≠Deny（092）/Ask→RequireConfirmation 映射测试 origin/scope 透传（093）/default_effect 三态+全同分首胜+restrictiveness 数值契约 4 用例（094）/fold+unwrap 链消除——单趟 (restrictiveness, priority) 字典序裁决（095）/matches Vec collect 消除——单次遍历零分配（096）/executor 全链路字段透传+覆盖注册+空白拒 3 用例（097）/阶段 4 策略强制真正接入——with_policy 挂载 ActionPolicy，Deny/Ask 在 handler 前拦截，此前 policy_check 空操作（098）/validate_schema 无谓 clone 消除 Valid 不携带 ParsedCommand（099） | 15 | cargo 311+5+4（+17）；clippy 0 警告；五门禁全绿 |
| RS4（RS 状态与会话层④——oracle/capability/broker/session_state/security_policy） | 9070996 | RS-100..115 16 项：oracle 有界 FIFO/多字段计数/空快照 4 用例（102：Vec::remove(0) O(n)→VecDeque pop_front O(1)）/Warning 变体落地——verify_with_warn_fields 唯一构造点，白名单外任一不匹配必须 Fail（101）/scope parse 全词表+风险排序+耗尽边界 5 用例（103）/白名单尾斜杠归一——带斜杠条目对裸 origin 静默失效（104）/uses_count 私有化+Capability::new 构造器（105）/register 返回被覆盖旧值——静默覆盖无审计痕迹（106）/broker 过期逐出+池满先清过期+同 id 续期 nonce 账本保留+nonce 长度边界 4 用例（107）/MAX_SESSIONS 过期驱动口径文档化（108）/奇数 hex+非 hex+缺必填字段拒绝（109）/**isIncognito 必填必须 bool——此前缺失/损坏静默降级普通标签（隐私语义丢失）**（110）/hex_encode write! 单缓冲（111）/保留名大小写+超长截断+多字节边界+url_decode 4 用例（112）/剥 RTL 双向控制符——RLO 扩展名伪装面（113）/**保留名检查截断后复查——CONX.ffff 截断后 base 变 CON 即绕过消解（真缺陷，测试驱动发现）**（114）/scheme 大小写变体 2 用例（115） | 16 | cargo 334+5+4（+23）；clippy 0 警告；五门禁全绿 |

| RS5（RS 浏览器功能层⑤——https_only/space_routing/command_bar/update_manifest/bridge_guard） | 27569e9 | RS-116..130 15 项：upgrade_counts 有界化 MAX_TRACKED_TABS=4096——池满新 tab 不入账+u64 饱和累加（116）/userinfo 剥离后放行判定——此前 http://user:pass@host 把凭据当域名+尾点域 fail-closed 升级 2 用例（117）/7 字节前缀比较替代全串 to_lowercase+双 find（118）/空 pattern 不命中——about:blank 无 host URL 此前被空域规则捕获+路径/精确保持大小写敏感+引擎回退 4 用例（119）/domain pattern 构造时小写归一（120）/strip_suffix+'.' 边界替代 ends_with(format!) 每匹配堆分配（121）/空集+max_results=0+Unicode 折叠 Ü/É/İ 口径锁定 4 用例（122）/JS search 补 value 通道——与 Rust matches() 口径对齐，此前两端漂移（123）/verify_threshold 白盒 4 用例——threshold 0/MAX/重复 keyid 只计一次/7 类畸形条目跳过/双密钥凑阈值+篡改失效（SigningKey 真实签名）（124）/version_tuple 预发布+u64 溢出+段数+u64::MAX 边界 4 用例（125）/canonical 键过滤遍历替代整 map clone（126）/**浮点 fail-closed 拒绝 CanonicalError——f64 to_string 与 Python json.dumps 字节级漂移（1e30 vs 1e+30）即签名验证失败面；canonical_unsigned 签名改 Result**（127）/base64 空串+URL-safe 字符+空白污染 3 用例（128）/scheme+host 大小写不敏感——HTTPS:// 与 Example.COM 不再误杀（129）/带端口字面 fail-closed+空白名单 require_https 组合 2 用例（130） | 15 | cargo 364+5+4（+30）；clippy 0 警告；五门禁全绿 |

| RS6（RS FFI 边界层⑥——bridge_guard/ffi/c_abi/broker/protection_mode） | 463937a | RS-131..145 15 项：bridge_guard new 补 ///（131）/FFI 导出层 3 用例——try_parse_external/extract_host/canonicalize（132）/build_fingerprint_pipeline 过期注释修正——JS 生成就 Rust 侧而非 Python legacy（133）/hex_seed 大写+nibble 序+00/ff/0f/f0 边界 3 用例（134）/FfiBroker 4 用例——destroy 清理双账本（非 nonce_replay）+reject/空 nonce fail-closed+待审批 1024 容量溢出拒+同 id 会话 replace 旧代际失效（RS-033 FFI 镜像）（135）/ledger_can_admit 原子性契约文档——检查+插入必须同锁窗口（拆分=TOCTOU 容量突破）+白盒直测 fresh-full 拒/过期清/Consumed 不清（136）/**generate_nonce 查表 hex 替代 32 次 format! 每导航堆分配**（137）/c_abi 5 用例——畸形 JSON 类型化拒+null 指针类型化码+retired u8 入口+u64::MAX 不 panic+FALLBACK 合法 JSON（138）/**FALLBACK deny 预构造 LazyLock\<CString\>——.expect() panic 路径消除；from_vec_unchecked 契约无内部 NUL（预拼 NUL 产生双 NUL）**（139）/**broker_new 进程级单例互斥 LIVE_BROKER——活跃期间二次创建 null（泄漏有界 1 活跃+1 遗留）；7 个 broker 测试加串行守卫**（140）/**read_utf8 细分错误码透传（ffi_input_null/too_long/utf8）——此前折叠 ffi_input_invalid 宿主无法区分**（141）/request_confirmation 文档错位修正——此前复制 consume 描述（142）/c_abi 3 用例——null 指针透传+极值组合+retired（143）/SessionContext 逐字段文档——created_at 单调时钟（144）/protection_mode 数字别名 0/1/2+空串空白 fail-closed 2 用例（145）；FFI 类型补 Debug derive | 15 | cargo 384+5+4（+20）；clippy 0 警告；五门禁全绿 |
| C8（CS P3 存储①——history/bookmark 存储层） | b56bd4a | CS-084..105 22 项：修剪逻辑 internal 阈值注入直测——常态 50k/256 离线不可测（084）/建目录静态记录仅一次（085）/Pooling=false 权衡注释（086）/**筛选子句单源 HistoryFilter.Build+Bind**——SearchRange/Count/SearchRangePage/SearchRangePaged 四处重复消除（087）/Read() 单调用者内联（088）/visited_date InvariantCulture——非公历默认日历文化（佛历/回历）年号漂移（089）/**visited_at 改 UTC round-trip——本地时字符串 DST 回拨时段字典序排序错乱；visited_date 仍本地日分组不变**（090）/双检锁换独立锁对象——不再锁 ConcurrentDictionary 实例（091）/LikeEscape 单遍扫描替代三连 Replace+字面量语义回归 8 用例（092）/NTP/geo host 大小写+子域不误拦用例（093）/临时文件清理捕 Exception——Delete 还可抛 UnauthorizedAccess 等（094）/空候选导入 (0,0) 用例（095）/ClampLimit 提取 internal 直测 6 边界（096）/空白候选不计 total 用例（097）/空白标题拒绝不落库用例（098）/**busy_timeout=5000 四库统一**——history/TabSession/DownloadRecord 补齐（099）/BookmarkImporter.Parse 损坏 JSON/缺失文件 fail-safe 返回空——与 HistoryImporter 同口径（100）/**TrimTitle 代理对安全截断——emoji 标题硬切劈半孤立代理乱码落库**（101）/深度>64 有界（200 层不溢出）+界内正路径用例（102）/无 roots 键用例（103）/URL 超 2048 拒收用例（104）/name 缺省回退 host——对齐 Python `name or host` 口径，此前整条丢弃（105） | 22 | Core.Tests 419+Broker.Tests 45 全绿；五门禁全绿 |
| C9（CS P3 存储②——downloads/favicon 缓存层） | 954e5d7 | CS-106..121 16 项：URL 单次解析——RequiresExplicitConfirmation 此前路径段与查询串各 TryCreate 一遍（106）/空 url+空文件名不命中用例（107）/多级扩展 x.tar.exe 取末段+tar 不误报用例（108）/URL 编码参数值 f=x%2Fy.exe 解码命中用例（109）/FormatBytes InvariantCulture——逗号小数文化显示漂移（110）/Refresh 兜底 catch——SDK 回调竞态异常面不再向 UI 定时器上抛（111）/**DownloadItemState 枚举单源——中文串既当存储又当显示，IsCompleted 比较依赖字面量，切换点拼写漂移即静默失配；StateText 单点映射+SetState 补 IsCompleted 通知**（112）/FormatBytes 提 internal+8 边界直测（113）/Open 失败连接 Dispose 对齐 BookmarkStore 口径（114）/EnsureSchema-once——此前每次 Add 都跑 CREATE TABLE（115）/非正 limit 钳 1——此前 SQLite 负值语义=无上限全表返回（116）/默认 limit=200 语义锁定（205 条→200）（117）/**仅超限修剪——COUNT 短路，未超上限不跑全表子查询 DELETE**（118）/**favicon 内存缓存命中写入后同样 Trim——此前仅 miss 路径修剪，全命中长会话只进不出**（119）/CachePath 提 internal——host 归一+sha1 十六进制文件名形态 2 测（120）/**内存缓存按持久化语义分面（Mem/PrivateMem）——无痕抓取不再写入普通窗口共享缓存**（121） | 16 | Core.Tests 440+Broker.Tests 45 全绿；五门禁全绿 |
| C10（CS P3 策略③——settings/policy/threat 层） | 9159c20 | CS-122..137 16 项：睡眠阈值白名单 static readonly 单源（122）/**窗口宽高默认+边界快照级常量单源——快照初始值与归一化回退两套并存消除**（123）/缩放双向钳制设置层用例（124）/构造器坏路径回退默认不抛用例（125）/Apply 恰通知一次 Changed 用例（126）/**SettingsService.FromPreloaded——组合根与服务此前各读一次同一 settings.json（启动链双读）**（127）/NaN 窗口位置持久化往返用例（128）/**ZoomStore.Set 的 Changed 统一锁外——归一移除分支持锁回调、写分支锁外回调两态并存；订阅者重入构成自锁死锁面**（129）/Load(null) 守卫——语义等价空表，此前 ArgumentNullException（130）/**LoadCached 回放套 ParseFeedLine——缓存被篡改/旧版脏条目不再绕过过滤入表**（131）/ReadBounded 超限返回 null 哨兵——此前 new byte[5MB+1] 只为表达一比特信息（132）/ThreatFeedTests+ThreatFeedLogicTests 合并为单一行为锁文件（133）/Coordinator 刷新直接用拉取返回值——此前写盘后重读磁盘（第三方改写即应用非拉取内容+多余全量 IO）（134）/**SecurityLog 体量计数器——此前每次写 File.Exists+FileInfo.Length 两次系统调用，进程首写校准一次后增量累计**（135）/**TrackerList.IsTracker 倒序集+span 备用查找——清单预展开为 reversed 单集，祖先域后缀零切片探测（镜像 CS-068 BlockedHosts 模式）**（136）/IsSameSite 尾字符判定——此前 "."+p 每调用拼接分配（137） | 16 | Core.Tests 438+Broker.Tests 45 全绿；五门禁全绿 |
| C11（CS P3 URL/建议/查找/审批④） | 2fed90a | CS-138..149+256 13 项：未知引擎回退默认引擎用例（138）/尾点输入判搜索词用例（139）/**裸 IPv6 字面量（::1 / [::1][:port][/path]）识别补 http+方括号归一——此前因不含点号被当搜索词；字母起始形如 scheme（fe80::1）维持 fail-closed 拒绝**（140）/ABOUT:BLANK 大小写用例（141）/host:port 带路径导航用例（142）/MergeRows OrdinalIgnoreCase 直判——此前每行两串 ToLowerInvariant 堆分配（143）/书签侧到量早退——此前仅历史侧 break（144）/seenUrls 去重忽略大小写——host 大小写变体不再重复占行（145）/WrapIndex 循环索引提纯+6 用例（146）/FindBar 空 catch 全吞→SecurityLog 留痕（147）/同标签二次 Request 不自拒+面板刷新用例（148）/审批过期文案 InvariantCulture——自定义格式串 ":" 为文化时间分隔符占位（149）/**Core.Tests 补 xunit.runner.json（与 Broker.Tests 同配置 parallelizeTestCollections=false）——修复 SettingsServiceTests 与 DecouplingTests 竞争 PrivacySettings 静态的既有 flake**（256） | 13 | Core.Tests 463+Broker.Tests 45 全绿（连跑两轮稳定）；五门禁全绿 |
| C12（CS P3 主窗收敛⑤——MainWindow.xaml.cs） | a7580bc | CS-150..158 9 项：窗口恢复阈值命名常量+回退宽高引用 CS-123 快照级单源（150）/CycleTab 就地循环定位——此前 ToList 全表复制只为找当前索引，每 Ctrl+Tab 一次（151）/**反馈条背景刷预建冻结**——此前每次显示 new 两把刷子（152）/**书签栏直显上限 20+溢出项「还有 N 条」跳书签管理器**——此前全部书签无上限重建挤压布局（153）/源码查看失败分支守卫 IsLoaded——对齐成功分支（154）/TruncateTitle 提 internal+代理对用例（155）/**FlushSession 收敛 OnClosed 单点——此前 Closing+Closed 双落盘（每次关闭双份 SQLite 写）**（156）/ToDigit 提纯+用例（157）/NextIndex 提纯+用例（158） | 9 | Core.Tests 477+Broker.Tests 45 全绿；五门禁全绿 |
| C13（CS P3 独立窗口⑥——history/bookmark/settings/downloads/datefield 窗口） | bb4b287 | CS-159..176 18 项：/**历史分页查询移后台线程+代际防回显**——此前 Count+页查询 UI 线程同步执行，大表翻页即冻结（159）/页码样式 TryFindResource 不抛+循环外提升（160）/TryHost 提 internal+4 用例（161）/书签过滤 OrdinalIgnoreCase 直判——此前每行两串 ToLowerInvariant（162）/编辑标题代理对安全截断（163）/**Rename/Remove/ClearAll 三路径库层异常捕获→可见反馈**——此前裸上抛炸窗口（164）/过滤谓词提纯 MatchesQuery 直测（165）/筛选态汇总改「匹配 N / 共 M」——此前「共 N」误导（166）/设置提示刷预建冻结——此前每次校验 new 两把（167）/KillSwitch 已触发文案单源（168）/SleepIndex 提 internal+6 边界（169）/下载轮询索引迭代——此前 ToList 每 500ms 全表复制（170）/**窗口隐藏暂停轮询、显示恢复**（171）/**危险扩展条目打开前二次确认**（172）/explorer /select 改 ArgumentList 传参——引号注入面消除（173）/清空列表按钮——仅移除已结束条目不删文件（XAML+处理器）（174）/DateField 文本 InvariantCulture——非公历默认日历年号漂移（175）/MondayOffset 提纯+用例（176） | 18 | Core.Tests 497+Broker.Tests 45 全绿；五门禁全绿 |
| C14（CS P3 WebView 环境与快照⑦） | c313427 | CS-177..188 12 项：InPrivate 标签切换补 IsHitTestVisible——对齐 MainWindow 口径（177）/**共享 WebView2 环境改 Lazy——`??=` 非原子，并发首开可双建环境（其一连同浏览器进程泄漏）**（178）/InPrivate 不注 DoH 定为既定口径并留注释（真机评估后可放开）（179）/临时目录清理重试全败→SecurityLog——残留可观测（180）/清理重试次数/间隔常量化（181）/指纹防护注入失败留痕——此前 fire-and-forget 静默失败=防护整段失效不可观测（182）/IsTrustedLocalHost 精确白名单 6 用例（183）/**指纹种子入口校验 fail-closed——64 位小写 hex 契约，非法种子此前直插 JS 单引号常量（注入面）**（184）/非法种子拒绝 7 形态用例（含注入形态）（185）/TabSessionStore SELECT 去掉未读取的 position 列（186）/空列表保存=清空会话用例（187）/is_current 标记损坏清零→末位标签兜底用例（188） | 12 | Core.Tests 511+Broker.Tests 45 全绿；五门禁全绿 |
| C15（CS P3 NTP/启动/宿主⑧） | bdf592a | CS-189..201 13 项：NTP 桥解析 catch 扩捕——超大数字 id 的 FormatException 此前接不住（189）/importHistory 字符串数字参数用例（190）/SavedSessionCount 改 COUNT 聚合——此前 hasSaved 全量 Load 只为数数（191）/FilterSources 大小写精确匹配语义锁定（192）/ResolveContentRoot 进程缓存——此前每标签创建 File.Exists 打盘（193）/IsVirtualHostUrl 直测 7 用例（194）/IsWallpaperAllowed 直测 6 用例（195）/PopupRateLimiter 频控提纯直测——此前内联静态数组（196）/启动链包 try——主窗构造失败 SecurityLog 留痕后 rethrow，此前闪退零痕迹（197）/pageHost 解析按来源串缓存——此前每子资源请求重 Parse（198）/文档代际饱和前置守卫——checked 溢出异常在导航事件链上即崩溃面（199）/**【真缺陷】Uri.Host 对 IPv6 保留方括号——方括号形态此前整体落到公网主机名兜底，链路本地/私网 bracketed IPv6 一律放行；剥离方括号按字面量判定**（200）/zone-id 形态（fe80::1%25eth0 等）拒绝——解析失败不再兜底放行（201） | 13 | Core.Tests 531+Broker.Tests 45 全绿；五门禁全绿 |
| C16（CS P3 死代码/原生桥/Broker⑨） | 3feba49 | CS-202..215 14 项：EnsureNotEngaged 死代码删除——零调用（202）/Contracts.cs 删除——ContractRoot+PolicyVersion 零引用且版本重复（203）/StorageService.cs 空骨架删除——零引用（204）/Tab 注释改为与 ImageSource 现实一致（205）/**多串 FFI 调用嵌套 try/finally——后续 Utf8 分配抛出时此前已分配指针泄漏（2/3/4 串三变体）**（206）/**裸 IntPtr 换 SafeHandle（NativeLibraryHandle/NativeBrokerHandle）——临界终结兜底 Dispose 遗漏；TryCreate 失败路径单点所有权移交释放**（207）/ParseDecisionPayload 未知决策 fail-closed 补测（ABI mismatch 既有覆盖核验）（208）/MaxSessions=1024（与 Rust 对等）/NativeSessionTtlSeconds=120/ActionLifetime=2min 常量单源（209/210/211）/broker 审计脱敏截断代理对安全——URL 含 emoji 硬切产生孤立代理落审计（212）/AuditLog 环形上限 5001 次事件断言 5000 条（213）/TryConsumeNavigation(null) 不抛拒绝（214）/不可解析 URL → url_policy 拒绝分支（215） | 14 | Core.Tests 531+Broker.Tests 49 全绿；五门禁全绿 |
| C17（CS P3 XAML 无障碍/键盘路径⑩） | e3819db | CS-216..229 14 项：CS-216/217/220/222 四项核验既有覆盖已足——EngineCombo/AddressBar/TabStrip/SourceText x:Name 已由先前批次补齐/NavButton Focusable=False 注释锁定既定取舍+键盘路径交叉引用（218）/FeedbackText 补 LiveSetting=Polite（219）/MainWindow AccentSoftBrush 删除——窗口内零消费方（其余窗口为各自本地副本+WindowTheme 运行时注入，不受影响）（221）/**无痕窗口补 Ctrl+L/Ctrl+T/Ctrl+W 快捷键——此前仅 Esc，无任何键盘操作路径**（223）/无痕窗口引擎由打开方传入——此前每窗同步读一次 settings.json（224）/书签编辑弹层键盘路径 Enter 保存/Esc 取消（225）/设置窗 Esc 关闭（226）/查找条收起显式还焦点页面 WebView——此前焦点悬空在已隐藏控件（227）/☆收藏排除内部虚拟主机页——地址仅本进程可解析（228）/虚拟主机页查看源码前置拦截——此前必 15s 超时后失败（229） | 14 | Core.Tests 531+Broker.Tests 49 全绿；五门禁全绿 |
| C18（CS P3 杂项收敛/可测性⑪） | ce7ed33 | CS-230..249 20 项：/**单实例命名 Mutex**——双开割裂会话/书签库，二次启动提示后退出（230）/ImportProbe 共享单源——History/Bookmark 两个 Importer 同形 DetectSources 近似复制消除（231）/订阅源 tmp 文件 Move 抛出时 finally 清理（232）/OnClosed 清 _sourceViewerWindows——Owner 引用驻留（233）/ApplyTheme 失败改 SecurityLog——此前仅 Debug.WriteLine（234）/SettingsWindow 复用 MainWindow.EngineOption——同形匿名类型双定义消除（235）/TabRuntime ZoomEpsilon 常量（236）/NavigationStarting 订阅命名化——匿名闭包无法退订（237）/**TabRuntimeCoordinator 同 id 重复创建先拆旧实例——此前直接覆盖，旧 runtime（WebView 控件+生命周期）泄漏**（238）/控制器构造器 ArgumentNullException 守卫测试（239）/TabSleepPolicy 空集合用例（240）/CS-241/242/244 三项核验既有覆盖已足（边界钳制含 -5.0、LoadCached 过滤、NoBlockedHosts 既有用例）/TrackerList.Domains 提 internal+清单元测试（全小写/唯一/逐条自命中——清单腐化报警面）（243）/空白订阅源地址视为未配置用例（245）/SecurityLog 目录注入面（246）+转义/截断直测（247）/AppPaths.ResetForTest+环境变量覆盖往返测试（248）/MainWindowDependencies 默认路径文档注明测试直接构造 record（249） | 20 | Core.Tests 539+Broker.Tests 49 全绿；五门禁全绿 |

| RS7（RS 工具链与边界层⑦——Cargo 五项/protection_mode/policy/broker/FFI/向量接入） | 289fe34 | RS-146..160 15 项（修正链 adffc88+cf801b9）：protection_mode 模式声明 Symbol 键收敛（RS-027 同款）——`__AEGIS_PROTECTION_MODE` 具名全局是指纹脚本免费探测点（泄漏防护存在性本身），改挂 Symbol.for 防篡改不可枚举 + MODE_SYMBOL 单源 3 用例（146）/**action-valid/invalid 向量 Rust 侧接入**——10 必填字段面映射 AuthorizedAction 构造锁定+method 词表常量对齐 schema enum+origin 归一/负代际 u64 拒绝/词表大小写敏感对齐；**新增 contracts/vectors/glob-match.json 33 向量**（match 24+subsume 9，PY-064 协议字段）消费测试——\*/\*\*/?/flat 门控/Unicode/后缀拼接绕过拒绝跨语言锁定（147）/**version_tuple 数字段收紧（真缺陷）**——u64::parse 接受 "+1" 与 "01"，此前 "+1.2.3" 与 "1.2.3" 解析出同一元组（版本比较可被混淆）、"01.2.3" 与 schema (0\|[1-9][0-9]\*) 跨语言漂移；strict_segment + 10 形态测试（148）/edition 2024 评估——uniffi 0.32 宏 metadata 注册静默失效（bindgen "No UniFFI metadata found"，本地测试全绿不暴露）**回退 2021**，#[unsafe(no_mangle)] 12 处保留零迁移（149）/[lints] 门禁——non_ascii_idents+unsafe_op_in_unsafe_fn+clippy correctness/suspicious deny（linker_messages allow=MSVC 本地化噪音）（150）/**[profile.release] lto=true+strip=debuginfo**——strip symbols 剥 UNIFFI_META 符号致 bindgen 失效（CI 实测回退口径）（151）/rust-version 1.87——edition 2024 线与已用 is_multiple_of（1.87）取最大+incompatible_msrv 门禁持续锁定（152）/getrandom 0.2→0.3 fill() 2 处（153）/**策略短路顺序计数 mock**——local→remote→failsafe 调用序此前仅来源断言（交换次序旧测试不失败）+本地命中远程零咨询（短路可观测）2 用例（154）/**broker 时钟单源 now_unix_secs**——validate_action 拆注入内核 validate_action_at，固定时刻锁边界不动系统时钟（155）/expires_at==now 即过期口径锁定（失效时刻非最后有效时刻）+now+1 窗口内+过去拒绝 3 用例（156）/H-7 注记三处统一——broker.rs 单一事实源，ffi 侧收敛为引用（157）/FFI create_session TTL 上限钳制 86400——u64::MAX 会话近乎永生扩大账本驻留面+core session_ttl 可观测 API+2 用例（C#/Android 120s 不受影响）（158）/FFI 空 session_id 拒绝（对齐 schema minLength 1，空 id 即匿名共享会话破坏 persona 隔离）+跨标签复用拒绝+同 id 续期 3 用例（159）/evaluate_navigation 三参数 clone 消除——move 进 action 每导航三次堆分配消除（160） | 15 | cargo 398+8+4（+14+3）；clippy 0 警告；五门禁全绿（修正链 2 波：edition 2024 回退+strip=debuginfo 根因定位） |

| RS8（RS 收官批——https_only/letterbox/command_bar/broker 注入/c_abi/adblock/fuzz 锁版） | fd152b9 | RS-161..205 45 项=实施 38+既往覆盖 7（169/174/187/195/198/202/204 经交叉核验由既有用例覆盖）：https_only allow_http **存储侧**大小写+尾点归一——查询侧归一会经 upgrade 判据放宽 RS-117 fail-closed 口径（存储/请求两侧同归一=行为变更），定向保持 + 存取往返 2 用例（161/162）/letterbox **逐属性 descriptor 存在性守卫断言**——screen 组 if(osX)、window 组 if(oX && oX.get) 双守卫+裸 defineProperty panic 扫描（防新增属性漏守卫）（163）/query_strip filter_query 提独立步骤 3 用例（164）/**command_bar max_results=0 上限失效（真缺陷）**——push 后判 ==max_results 对 0 永假，上限静默失效空查询吐全量；提前返回空集 + MAX_FIELD_CHARS=512 字符级截断（UTF-8 安全）+ search 显式短路 + 8 内置操作全量断言（165/193/194）/executor 三枚举契约测试（166）/policy 字段文档+FailSafe Deny explanation 扩展断言+访问器突变测试（167/182/201）/**matcher "\*?" 最少消耗断言修正**——Star 允许空匹配（"\*\*\*" 匹配空串已证），最少 1 字符非 2；三星/星问组合+flat 锁步 subsume 测试（168/192）/capability 7 方法 /// 文档（170）/**broker 三件**——MAX_CONSUMED_NONCES cfg(test) 注入 256 账本满 fail-closed 用例（171）/generation u64::MAX checked_add 溢出分支用例（183）/**is_expired_at 注入时钟内核**——saturating_duration_since 防时钟回拨 panic+==ttl 未过期边界锁定（203）/session_state 版本 0/2 拒绝+hex 迁 util 单源（172/186）/ffi MAX_FFI_URL_BYTES=64KB 三入口前置门禁（173）/**c_abi**——write_response to_writer 单缓冲（175）/**NATIVE_BOUNDARY_EXPLANATION 常量单源**+FALLBACK_JSON 字节包含同步性断言（静态字面量无法引用运行时常量，测试锁死）（176）/CAbiBroker pub 可达性说明（200）/origin 四字段文档+方括号 IPv6 拒绝（177/190）/**util hex 编解码单源**+非 ASCII 字节拒绝+0..=255 全遍历往返（178/186）/**adblock 入库归一（fail-open 修复）**——黑名单条目 trim+lowercase+尾点剥离，此前大小写变体条目永不相等形同虚设+幂等/空白拒绝 3 用例（179/180）/js_iife! 宏字面量约束文档（181）/**security_policy 控制空白剥离扩展 \n\r\t**——文件名内嵌换行视觉伪装面（188）/protection_mode 3 模式全覆盖互异测试（189）/decision 11 字段文档（191）/space_routing 空/无 host 回落默认测试——伪主机名 "about"/纯空白透传为 util 既有口径锁定（uniffi 契约不变更）（196）/**update_manifest try_verify_signature 提取**——? 链替代 8 层 continue 嵌套（197）/**ffi/broker matches_navigation_binding 单源**——approve/consume 两处四属性内联收敛+generate_nonce 拆分（entropy_error 映射+nonce_from_entropy 确定性契约）+畸形 URL approve 后 nonce 一次性语义用例（184/185/199）/**fuzz libfuzzer-sys 锁 0.4.13**——与 Cargo.lock 一致 minor 漂移禁入（205） | 45 | cargo test 427/0（+29）；fmt+clippy -D warnings 0 警告；fuzz check --bins 6 target 通过；五门禁全绿 |

> 完成度：705 项落地（跨 46 批——先导/R1/C1/W1/P1/A1/W2/W3+SP1/A2/A3/A4/W4/R1+/R1++/C2+C3/C4/C5/C6/C7/PY1/PY2/PY3/PY4/PY5-1/PY5-2/PY5-3/PY5-4/RS1/RS2/RS3/RS4/RS5/RS6/C8/C9/C10/C11/C12/C13/C14/C15/C16/C17/C18/RS7/RS8；beta.44–49 各批随版本递增，其后批次独立提交），并随批补强回归/行为测试；1115 项完整清单作为后续批次的工作池（余量 ~410 项：CS-250..296 / WB-031..100 / AD-070..210 / SP-009..138；另有 AD-003/005 与 AD-067 冒烟执行共 3 项暂缓待真机）。**RS 面（RS-001..205）全量闭环。**
