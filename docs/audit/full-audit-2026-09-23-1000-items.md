...
| 暂缓 | — | AD-003（R8 minify）、AD-005（per-site SHA-256）——需真机回归，按 device-validation runbook 单独批次；AD-035/038..047/064/067（P2 UI 批次：库层错误码上抛/collectAsStateWithLifecycle/itemsIndexed key/semantics/strings.xml 迁移/canGoBack Flow/androidTest 冒烟）——下一批次 | 2+13 | 待下批 |
