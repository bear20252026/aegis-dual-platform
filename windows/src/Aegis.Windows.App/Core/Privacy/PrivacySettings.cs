...
/// 安全 DNS 变化需重启生效（环境参数构建一次）。
/// 字段 volatile：设置窗口（UI 线程）写、威胁源刷新（线程池）与导航事件
///（WebView 线程）读之间保证可见性。</summary>
public static class PrivacySettings
{
    /// <summary>跟踪防护级别：0 基础 / 1 均衡 / 2 严格。</summary>
    public static volatile int ProtectionLevel = 1;

    /// <summary>HTTPS-only：http 自动升级 https（无 https 的站点会失败并提示）。</summary>
    public static volatile bool HttpsOnly = true;

    /// <summary>安全 DNS（DoH）：随环境参数生效，改动需重启。</summary>
    public static volatile bool SecureDns = true;
}
